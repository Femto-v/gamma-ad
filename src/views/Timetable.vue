...
const filteredSubjects = computed(() =>
    subjectList.value.filter(
        (s) => s.semester === activeSemester.value && s.sesi === activeSesi.value
    )
);
...
const daySchedule = computed(() => {
    ...
    timetableData.value.forEach((row, timeIndex) => {
        ...
        if (daySlot && daySlot.trim()) {
            ...
            const subjectInfo = filteredSubjects.value.find(s => s.kod_subjek === subjectCode);
            ...
            if (!subjectColorMap.has(subjectCode)) {
                subjectColorMap.set(subjectCode, subjectColors[colorIndex % subjectColors.length]);
                colorIndex++;
            }
...
<template>
    <div class="bg-gray-50 min-h-screen">
            ...
            <div class="px-4 py-2">
                ...
                <div class="flex justify-between mb-4">
                    <div 
                        ...
                        :class="selectedDay === index ? 'bg-purple-100' : 'hover:bg-gray-100'"
                    >
                        <span class="text-xs text-gray-500 mb-1">{{ day.substring(0, 3) }}</span>
                        <div 
                            class="w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium"
                            :class="selectedDay === index ? 'bg-purple-500 text-white' : 'text-gray-700'"
                        >
                            {{ index + 1 }}
                        </div>
                    </div>
        ...
        <div class="px-4 py-4">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-gray-800">Today</h3>
            </div>
            ...
            <div class="space-y-3">
                <div 
                    ...
                    :class="item.color"
                >
                    <div class="flex flex-col items-center text-xs text-gray-600 min-w-[50px]">
                        <span class="font-medium">{{ item.time.split('-')[0] }}</span>
                        <span class="text-gray-400">{{ item.time.split('-')[1] }}</span>
                    </div>
                    
                    <div class="flex-1">
                        <h4 class="font-medium text-gray-800 mb-1">{{ item.subject }}</h4>
                        <p class="text-sm text-gray-600 mb-2">{{ item.subjectName }}</p>
                ...
                <div v-if="daySchedule.length === 0" class="text-center py-8">
                    ...
                    <p class="text-gray-500 text-sm">No classes scheduled for {{ days[selectedDay] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 py-6 text-center">
            <p class="text-xs text-gray-500">
                ...
                <a href="mailto:ttms@fc.utm.my" class="text-purple-600 hover:underline">ttms@fc.utm.my</a><br />
