...
async function fetchSesiSemesterOptions() {
    try {
        // You may want to adapt this if your API is different!
        ...
        if (Array.isArray(sesiSemesterData) && sesiSemesterData.length) {
            ...
            semesterOptions.value = [
                ...new Set(sesiSemesterData.map((item) => item.semester)),
            ];
            // Set default to latest
            selectedSesi.value =
                sesiOptions.value[sesiOptions.value.length - 1];
            selectedSemester.value =
                semesterOptions.value[semesterOptions.value.length - 1];
