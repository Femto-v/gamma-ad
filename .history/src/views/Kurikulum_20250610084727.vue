...
onMounted(async () => {
    ...
    try {
        ...
        const cohortPromises = sesiList.map(async (item) => {
            try {
                const cohort = await api.getCohort(item.sesi_masuk);
                if (!cohort || !cohort.nama_kurikulum) return null; // skip invalid
                return {
                ...
                console.error("Error for cohort", item.sesi_masuk, e);
                return null;
            }
        });
