    ...
    try {
        ...
        do {
            ...
            if (Array.isArray(batch) && batch.length > 0) {
                allStudents.push(
                    ...batch.map((item) => ({
                        no_matrik: item.no_matrik, // <-- make sure this is included!
                        name: item.nama,
...
// Button click: Pass correct no_matrik
function goToStudentTimetable(matric) {
    if (!matric) return;
    router.push({
        name: "TimetableAnotherStudent",
        query: { matric }, // This will set ?matric=xxxx
    });
}
