...
<template>
    ...
    >
        <div class="bg-white rounded-xl shadow p-6 max-w-xl w-full relative">
            <button
                @click="closeModal"
                class="absolute top-2 right-2 p-2 rounded hover:bg-gray-100"
            >
                ✕
            </button>
            <h3 class="font-bold text-lg mb-2">Subject List</h3>
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr
                        v-for="s in selectedSubjects"
                        :key="s.id_kurikulum_subjek"
                    >
                        <td>{{ s.kod_subjek }}</td>
                        <td>{{ s.nama_subjek }}</td>
                        <td>
                            {{
                                s.nama_elektif || s.mod_elektif
                                    ? "Elektif"
                                    : "Teras"
                            }}
                        </td>
