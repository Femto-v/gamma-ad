...
// Handle horizontal overflow/canvas width
...
// On load: get session and all students
const loadAllStudents = async () => {
    ...
    try {
        ...
        nextTick(() => {
            updateOverflow();
            scrollToCard(0); // center first card
        });
    } catch (err) {
        students.value = [];
...
// Validate session and fetch students
...
watch(filteredStudents, updateOverflow);
</script>
    ...
    <div class="bg-transparent min-h-screen flex flex-col">
        ...
        <div v-else class="w-full flex flex-col items-center px-0">
            <div class="relative w-full max-w-full mx-auto select-none">
                <button
                    class="absolute left-1 top-1/2 -translate-y-1/2 z-10 bg-white/80 border shadow p-1 rounded-full hover:bg-blue-50 transition text-xs"
                ...
                >
                    <div
                        ...
                        :key="idx"
                        class="w-[220px] min-w-[220px] max-w-[220px] h-[350px] snap-center bg-blue-100 rounded-2xl shadow-sm p-6 flex flex-col items-start relative transition-all duration-500 border border-blue-100"
                        :style="{
                            transform:
                    ...
                    >
                        ...
                        <div
                            class="font-semibold text-lg mt-2 mb-2 w-full text-blue-900 break-words leading-tight"
                ...
                <button
                    class="absolute right-1 top-1/2 -translate-y-1/2 z-10 bg-white/80 border shadow p-1 rounded-full hover:bg-blue-50 transition text-xs"
                    :disabled="
...
<style scoped>
/* Custom for smoother card shadow, no hard edge on border */
.shadow-sm {
    box-shadow: 0 2px 10px 0 rgba(59, 130, 246, 0.09);
}
</style>
