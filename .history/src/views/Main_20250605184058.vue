...
creditInfo.value = computed(() =>
    subjectsWithCredit.value.reduce((sum, subj) => sum + subj.kredit, 0)
);
...
onMounted(async () => {
    ...
    const dataSubject = await pelajarSubjekApi.getPelajarSubjects(noMatrik);
    subjects.value = Array.isArray(data) ? data : [data];
});
</script>
